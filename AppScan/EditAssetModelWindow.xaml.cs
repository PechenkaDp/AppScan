...
namespace AppScan
{
    public partial class EditAssetModelWindow : Window
    {
        private AssetModel _assetModel;

        public EditAssetModelWindow(AssetModel assetModel)
        {
            InitializeComponent();
        ...
        {
            NameTextBox.Text = _assetModel.Name;
            BarcodeTextBox.Text = _assetModel.Barcode;
            SpecificationsTextBox.Text = _assetModel.Specifications;
            ManufacturerComboBox.SelectedValue = _assetModel.ManufacturerId;
            AssetTypeComboBox.SelectedValue = _assetModel.AssetTypeId;
        }
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValidateInput())
            {
                ...
                bool success = DatabaseHelper.UpdateAssetModel(_assetModel.ModelId, name, manufacturerId, assetTypeId, specifications, barcode);

                if (success)
                {
                    MessageBox.Show("Модель актива успешно обновлена", "Успех", MessageBoxButton.OK, MessageBoxImage.Information);
                    DialogResult = true;
                    Close();
                }
                else
                {
                    MessageBox.Show("Ошибка при обновлении модели актива", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
