...
namespace AppScan
{
    public partial class EditRoomWindow : Window
    {
        ...
        private void PopulateFields()
        {
            ...
            FloorTextBox.Text = _room.Floor.ToString();
            BuildingComboBox.SelectedValue = _room.BuildingId;
        }
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValidateInput())
            {
                ...
                bool success = DatabaseHelper.UpdateRoom(_room.RoomId, roomNumber, floor, buildingId);

                if (success)
                {
                    MessageBox.Show("Помещение успешно обновлено", "Успех", MessageBoxButton.OK, MessageBoxImage.Information);
                    DialogResult = true;
                    Close();
                }
                else
                {
                    MessageBox.Show("Ошибка при обновлении помещения", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
